...
import { getRandomParagraph, getRandomText } from "@/lib/paragraphs";
import {
    calculateGrossWPM,
    calculateNetWPM,
    calculateAccuracy,
...
// Average typist ~40 WPM = ~200 chars/min. We generate extra buffer.
function charsForDuration(seconds: number): number {
    return Math.ceil((seconds / 60) * 300);
}
...
export default function TypingTest() {
    ...
    // Generate text appropriate for the selected duration
    const generateText = useCallback((dur: number) => {
        if (dur <= 60) return getRandomParagraph();
        return getRandomText(charsForDuration(dur));
    }, []);

    // Initialize paragraph on mount
    useEffect(() => {
        setParagraph(generateText(duration));
    ...
    const handleRestart = useCallback(() => {
        stopTimer();
        setParagraph(generateText(durationRef.current));
        setTyped("");
    ...
    }, [stopTimer, generateText]);

    const handleDurationChange = useCallback(
        (newDuration: number) => {
            if (hasStarted) return; // Prevent changing mid-test
            setDuration(newDuration);
            setTimeLeft(newDuration);
            setParagraph(generateText(newDuration));
            setTyped("");
            setIsFinished(false);
            startTimeRef.current = 0;
            setTimeout(() => inputRef.current?.focus(), 50);
        },
        [hasStarted, generateText]
    );
    ...
    useEffect(() => {
        if (cursorRef.current && containerRef.current) {
            ...
            // Use scrollIntoView with block: 'center' to keep it nicely centered
    ...
    // Track cursor position to dynamically place the hidden input under it 
    // so the browser's native focus-scroll behavior doesn't jump the view to the top
                ...
                <div className="inline-flex items-center gap-1 p-1 rounded-xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 transition-colors duration-300">
                    {DURATION_OPTIONS.map((opt) => (
                        <button
                            ...
                            `}
                            aria-label={`Set test duration to ${opt.label}`}
                        >
                            {opt.label}
                        </button>
                    ))}
